   ...
   function addnewuser($name, $password, $email, $type)
{
    try {
        ...
        $stmt = $this->connection->prepare(
            "INSERT INTO userdata (name, email, pass, type) VALUES (:name, :email, :pass, :type)"
        );
        ...
        if ($stmt->execute()) {
            ...
            exit(); // Make sure script stops after header
        } else {
            header("Location: admin_service_dashboard.php?msg=Register Fail");
            exit();
        }
    } catch (PDOException $e) {
        // Optional: log the error and redirect
        error_log($e->getMessage());
        header("Location: admin_service_dashboard.php?msg=Error Occurred");
        exit();
    }
}
